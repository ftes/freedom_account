defmodule FreedomAccountWeb.TransactionLive.TransactionFormTest do
  ...
  describe "updating account transactions" do
    test "edits single-fund transaction", %{conn: conn, fund: fund} do
      ...
      |> assert(account_balance() |> by_css() |> filter(has_text: html_text(MoneyUtils.format(new_amount))) |> visible())
      ...
      |> assert("in" |> role() |> by_css() |> filter(has_text: html_text(MoneyUtils.format(new_amount))) |> visible())
    end
    ...
    test "edits multi-fund transaction", %{account: account, conn: conn, fund: fund1} do
      ...
      |> assert(table_cell() |> by_css() |> filter(has_text: html_text("#{new_date}")) |> visible())
      |> assert(table_cell() |> by_css() |> filter(has_text: html_text(new_memo)) |> visible())
      |> assert("in" |> role() |> by_css() |> filter(has_text: html_text(MoneyUtils.format(net_amount))) |> visible())
    end
    ...
    test "edits loan transaction", %{conn: conn, loan: loan} do
      ...
      |> assert(table_cell() |> by_css() |> filter(has_text: html_text("#{new_date}")) |> visible())
      |> assert(table_cell() |> by_css() |> filter(has_text: html_text(new_memo)) |> visible())
      |> assert("out" |> role() |> by_css() |> filter(has_text: html_text(MoneyUtils.format(new_amount))) |> visible())
    end
