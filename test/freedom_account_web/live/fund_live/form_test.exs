defmodule FreedomAccountWeb.FundLive.FormTest do
  ...
  describe "creating a new fund" do
    test "saves new fund", %{conn: conn} do
      ...
      |> visit(~p"/funds/new")
      |> assert(page_title_contains("Add Fund"))
      |> assert(heading() |> by_css() |> filter(has_text: html_text("Add Fund")) |> visible())
      |> fill(by_label("Icon", exact: true), to_string(""))
      |> fill(by_label("Name", exact: true), to_string(""))
      |> assert("#fund_icon" |> field_error() |> by_css() |> filter(has_text: html_text("can't be blank")) |> visible())
      |> assert("#fund_name" |> field_error() |> by_css() |> filter(has_text: html_text("can't be blank")) |> visible())
      ...
      |> assert(:info |> flash() |> by_css() |> filter(has_text: html_text("Fund created successfully")) |> visible())
      |> assert(fund_icon() |> by_css() |> filter(has_text: html_text(icon)) |> visible())
      |> assert(fund_name() |> by_css() |> filter(has_text: html_text(name)) |> visible())
      |> assert(fund_budget() |> by_css() |> filter(has_text: html_text("#{budget}")) |> visible())
      |> assert(fund_frequency() |> by_css() |> filter(has_text: html_text("#{times_per_year}")) |> visible())
      |> assert(fund_balance() |> by_css() |> filter(has_text: html_text("$0.00")) |> visible())
    end
    ...
    test "does not create fund on cancel", %{conn: conn} do
      ...
      |> assert(fund_name() |> by_css() |> filter(has_text: html_text(name)) |> count(0))
    end
  end
  ...
  describe "editing a fund" do
    test "updates fund settings", %{account: account, conn: conn} do
      ...
      |> assert(page_title_contains("Edit Fund"))
      |> assert(heading() |> by_css() |> filter(has_text: html_text("Edit Fund")) |> visible())
      |> fill(by_label("Icon", exact: true), to_string(""))
      |> fill(by_label("Name", exact: true), to_string(""))
      |> assert("#fund_icon" |> field_error() |> by_css() |> filter(has_text: html_text("can't be blank")) |> visible())
      |> assert("#fund_name" |> field_error() |> by_css() |> filter(has_text: html_text("can't be blank")) |> visible())
      ...
      |> fill(by_label("Times/Year", exact: true), to_string(times_per_year))
      |> click(by_role(:button, name: "Save Fund"))
      |> assert(:info |> flash() |> by_css() |> filter(has_text: html_text("Fund updated successfully")) |> visible())
      |> assert(fund |> fund_icon() |> by_css() |> filter(has_text: html_text(icon)) |> visible())
      |> assert(fund |> fund_name() |> by_css() |> filter(has_text: html_text(name)) |> visible())
      |> assert(fund |> fund_budget() |> by_css() |> filter(has_text: html_text("#{budget}")) |> visible())
      |> assert(fund |> fund_frequency() |> by_css() |> filter(has_text: html_text("#{times_per_year}")) |> visible())
      |> assert(
        fund
        |> fund_balance()
        |> by_css()
