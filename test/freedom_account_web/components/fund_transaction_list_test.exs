defmodule FreedomAccountWeb.FundTransactionListTest do
  ...
  describe "Index" do
    ...
    test "allows editing transaction in listing", %{conn: conn, fund: fund} do
      ...
      |> assert(page_url(~p"/funds/#{fund}/transactions/#{deposit}/edit"))
    ...
    test "deletes transaction in listing", %{conn: conn, fund: fund} do
      ...
      |> assert(fund |> sidebar_fund_balance() |> by_css() |> filter(has_text: html_text("$0.00")) |> visible())
      |> assert(count(by_css("#txn-#{line_item.id}"), 0))
    end
